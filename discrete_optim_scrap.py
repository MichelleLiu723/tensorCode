        ...
        for k in range(1):   
            ...
            for i in range(len(currentNetwork)):
                ...
                for j in range(i, len(currentNetwork)):
                    ...
                    stop_cond = generate_stop_cond(cc.get_indims(currentNetwork))
                    ...
                    [curentNetwork, first_loss, current_loss] = cc.continuous_optim(currentNetwork, train_data, loss_fun, epochs=10, val_data=None, other_args=dict())
                    if prev_loss > current_loss:
                        better_network = currentNetwork
                        numParam = cc.num_params(better_network)
                        better_loss = current_loss
                    else:
                        print('prev_loss < current_loss so no rank update at this iteration')
                    #reset currentNetwork to contuniue with greedy at another point
                    curentNetwork = cc.copy_network(initialNetwork)
            #upate parameters
